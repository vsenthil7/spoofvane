...
| UX-1 | **Account menu: logout + role switch + tenant switch** | ⬜ PLANNED | The console boots into the shell with NO logout, the top-bar role chip is display-only (can't switch role without a code hot-restart), and the `AcmeBank ▾` tenant dropdown is a decorative label (not wired). Build a real top-right account menu: shows current user/role/tenant, lets you switch role (re-route via the existing `Console(role:)`), switch tenant, and **Log out** (navigate back to `LoginScreen`, clear session state). Login + role routing already exist, so this is small. Found live by the product owner (30 May, 23:10) — an enterprise product shipping RBAC/MFA/SoD with no logout is a hard reviewer flag. |
