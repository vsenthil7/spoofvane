...
| DEMO-3 | **Route the live scan through the formal agent framework** | ⬜ PLANNED | Owner raised (30 May, 23:36): the hackathon wants an **agent**, and the live scan is currently a direct multi-LLM ensemble call, not routed through the real agent framework (E1–E10: `src/agents/`, governance, kill-switch, audit). Wire the scan as an agent action so it runs under the existing GovernanceEngine (budget, lawful-basis, two-person gate where relevant) + KillSwitch + agent_audit, and is visible as an agent run. Makes the live demo agentic end-to-end, not just multi-LLM. |
